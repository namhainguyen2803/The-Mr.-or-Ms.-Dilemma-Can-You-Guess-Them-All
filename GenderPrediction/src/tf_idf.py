...
class Compute_TF_IDF():
    def __init__(self, list_sentence, dictionary=None, max_count=None, min_count=None, normalize_tf=False, smooth=True,
        ...
        self.num_sentence = len(self.list_sentence)
    ...
    def map_word_to_count(self):
        ...
        for i in range(len(self.list_sentence)):
            list_word = self.word_extraction(self.list_sentence[i].lower())
    ...
    def create_dictionary(self):
        if self.max_count == None and self.min_count == None:
            set_word = set()
            for sentence in self.list_sentence:
                set_word = set_word.union(set(self.word_extraction(sentence.lower())))
        else:
            set_word = set()
            mapping_word_count = self.map_word_to_count()
            for sentence in self.list_sentence:
                list_word = self.word_extraction(sentence)
    ...
    def create_count_matrix(self):
        mat = np.zeros((self.num_sentence, self.num_word))
        for i in range(len(self.list_sentence)):
            sentence = self.list_sentence[i].lower()
            list_word = self.word_extraction(sentence)
            for j in range(len(list_word)):
    ...
    def compute_idf(self):
        ...
        if self.smooth == True:
            ...
            num_doc_having_word = np.log((self.num_sentence + 1) / (num_doc_having_word + 1)) + 1
        else:
            # avoid 0 in idf
            num_doc_having_word = np.log(self.num_sentence / num_doc_having_word) + 1
